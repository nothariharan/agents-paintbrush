/**
 * Paint-overflow divider: previous section colour drips into the next.
 * `from` / `to` must match the solid backgrounds of the adjacent sections
 * exactly — no translucent section fills, or colours will look overlapped.
 */
...
/** Solid section colours used by both drips and sections — keep in sync. */
...
export function PaintDrip({
  ...
  from: keyof typeof SECTION | string;
  to: keyof typeof SECTION | string;
  className?: string;
}) {
  const fromColor = SECTION[from] ?? from;
  const toColor = SECTION[to] ?? to;

  return (
    <div
      aria-hidden="true"
      className={`relative block leading-[0] ${className ?? ""}`}
      style={{ backgroundColor: toColor }}
    >
      {/*
        Layer model (no overlap):
        1) wrapper = next section colour (solid)
        2) drip path + drops = previous section colour (solid)
        3) ink stroke sits on the boundary only
      */}
      <svg
        viewBox="0 0 1440 170"
        preserveAspectRatio="none"
        className="block h-[4.5rem] w-full sm:h-24"
        shapeRendering="geometricPrecision"
      >
        <path d={DRIP} fill={fromColor} />
        {BLOBS.map(([cx, cy, r]) => (
          <ellipse key={`f-${cx}`} cx={cx} cy={cy} rx={r} ry={r * 1.25} fill={fromColor} />
        ))}
        <path
          d={DRIP_EDGE}
          fill="none"
